                ...
                {items.map(item => (
                    <div className="flex flex-col border-b border-gray-500 py-4">
                        <div className="flex flex-row justify-center">
                            <div className={`rounded-badge ${item.compliant ? 'bg-low' : 'bg-high'} w-4 h-4`}></div>
                            <div className="flex-grow mx-2 text-bright">{item.title}</div>
                            <div className="text-right">Activity: <span
                                className="text-bright">{item.activityTitle}</span></div>
                        </div>
                        <div key={item.id} className="flex flex-row mt-4">
                            <div className="flex-grow ml-6 mx-2">{item.description}</div>
                            <div className="flex flex-col text-right w-64">
